...
async function run() {
  ...
  const sql = await readFile(new URL('../sql/create_eventos_table.sql', import.meta.url), 'utf8');
  ...
  const tableCheck = await client.query(
    "SELECT to_regclass('public.eventos') AS tabela, COUNT(*)::int AS total FROM public.eventos"
  );
